package com.allevite.part2_WellKnownTypeclass.chapt12_Excercise
...
object Get :
  ...
  given monadGet: MonadError[Get, String] = new MonadError[Get, String] :
    ...
    override def tailRecM[A, B](a: A)(f: A => Get[Either[A, B]]): Get[B] =
      Get : bytes =>
        Monad[[z] =>> Either[String, z]].tailRecM((bytes, a)) { case (bytes, a) =>
   ...
   */
  given eqGet[A: Eq]: Eq[Get[A]] = Eq.instance { (g1, g2) =>
    val nSamples = 32
   ...
   */
  given monoid[A: Monoid]: Monoid[Get[A]] = new Monoid[Get[A]] {
    /**
