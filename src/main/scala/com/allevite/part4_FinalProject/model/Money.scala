...
import com.allevite.part4_FinalProject.model.Money.showMoney
...
object Money :
  ...
  given monoidMoney: Monoid[Money] = Monoid.instance(zero, _ plus _)

  given showMoney: Show[Money] = Show.show(m => f"$$${m.dollars}%.2f")

  given eqMoney(using eqInt: Eq[Int]): Eq[Money] =
   ...
   */
  given orderMoney(using orderInt: Order[Int]): Order[Money] = Order.by(_.cents)
