package com.allevite.part4_FinalProject.common
...
object IO :
    ...
    io match {
      ...
      case FlatMap(t, f) =>
        t match {
          ...
          case FlatMap(t2, f2) =>
            resume(FlatMap(t2, (x) => FlatMap(safeApply(f2(x)), f)), eh)
        }
    }
  ...
  given ioMonadError: MonadError[IO, Throwable] = new MonadError[IO, Throwable] :
