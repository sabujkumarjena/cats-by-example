package com.allevite.part3_FunctionalTechniques.chapt4_StateManagement
...
object TicTacToeApp :
  ...
  object Player {
    ...
    given showPlayer: Show[Player] = Show.show {
      case P1 => "X"
      case P2 => "O"
    }
   ...
   given  eqPlayer: Eq[Player] = Eq.fromUniversalEquals[Player]
  }
  ...
  object Board :
    ...
    given showBoard: Show[Board] = Show.show { board =>
  ...
  object GameState:
    given  gameStateShow: Show[GameState] = Show.show { gs =>
      s"""Turn: ${gs.turn}
         |
         |Board:
         |${gs.board.show}
  ...
  class TicTacToeVar(boardSize: Int):
    var board: Board = Board.empty(boardSize)
    var turn: Player = Player.P1
    def currentPlayerWon(): Boolean = ???
    def switchTurn(): Unit =
      turn = if turn === Player.P1 then Player.P2 else Player.P1
    def doPlay(play:Play): Unit =
      board = board.updated(
        play.row, play.col, turn
      )
